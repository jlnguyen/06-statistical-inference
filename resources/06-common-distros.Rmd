...
# x expressed ito standard deviations (xSd)
xSd <- (x - mu)/sd; xSd
pnorm(xSd, lower.tail = FALSE)
```
...
dat <- data.frame(
    x = xvals,
    y = ppois(xvals, lambda = 2.5 * 4),
    pmf = dpois(xvals, lambda = 2.5 * 4)
)
ggplot(dat, aes(x = x, y = y)) + geom_line(size = 1)
ggplot(dat, aes(x = x, y = pmf)) + geom_line(size = 1)
```
