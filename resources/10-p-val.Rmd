...
#### Joe | Sun | 25 Oct, 2015
For two-sided p-value, compute p-values for (children >=7) and (children <=7), get min, and double.
```{r}
pval1 <- pbinom(6, size = 8, prob = .5, lower.tail = FALSE); pval1
pval2 <- pbinom(7, size = 8, prob = .5, lower.tail = TRUE); pval2
pval <- 2 * min(c(pval1, pval2)); pval
```
